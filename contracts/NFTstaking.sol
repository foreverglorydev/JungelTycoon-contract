...
contract JungelTycoon is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function withdraw() public payable onlyOwner {
    // This will pay Dev 2% of the initial sale.
    // =============================================================================
    (bool hs, ) = payable(0x49D23d4aD43FBAbd5e9f268c115A5E6e4d17ED5E).call{value: address(this).balance * 2 / 100}("");
    require(hs);
    // =============================================================================
...
contract JTT is ERC20, Ownable {

  // a mapping from an address to whether or not it can mint / burn
  mapping(address => bool) controllers;
  
  constructor() ERC20("JTT", "Jungle Tycoon Token") { }

  /**
   * mints $JTT to a recipient
   * @param to the recipient of the $JTT
   * @param amount the amount of $JTT to mint
   */
  ...
  /**
   * burns $JTT from a holder
   * @param from the holder of the $JTT
   * @param amount the amount of $JTT to burn
   */
...
contract Gallery is Ownable, IERC721Receiver {
    ...
    * @param _jtt reference to the $JTT token
    */
    constructor(address _jungle, address _jtt) { 
        jungle = JungelTycoon(_jungle);
        jtt = JTT(_jtt);
    }
    ...
    */
    function withdrawFromGallery(uint16[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            for (uint j = 0; j < stakedTokens[_msgSender()].length; j++) {
                if (tokenIds[i] == stakedTokens[_msgSender()][j]) {
    ...
    function onERC721Received(
        ...
        require(from == address(0x0), "Cannot send tokens to Gallery directly");
